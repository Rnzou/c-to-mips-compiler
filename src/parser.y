...
%token T_ASSIGN T_INCREMENT T_DECREMENT T_ADD_ASSIGN T_SUB_ASSIGN T_MULTIPLY T_DIVIDE T_PLUS T_MINUS T_MODULO
...
%token T_GREATER T_GREATER_EQUAL T_LESS T_LESS_EQUAL T_EQUAL T_NOT_EQUAL T_IF T_ELSE T_SWITCH T_WHILE T_FOR T_CONTINUE T_BREAK T_RETURN T_DEFAULT T_CASE
%token T_INT T_VOID T_CHAR T_DOUBLE T_FLOAT T_UNSIGNED T_IDENTIFIER T_INT_CONSTANT T_FLOAT_CONSTANT T_DOUBLE_CONSTANT T_UNSIGNED_CONSTANT
...
%type <expression_node> logical_expression  
...
global_declaration				:	function_def	{ $$ = $1; }
								|	declaration 	{ $$ = $1; }
								|	enumerator	{ $$ = $1; }

function_def				    :	TYPE T_IDENTIFIER T_LBRACKET parameter_list T_RBRACKET compound_statement	{ $$ = new Function_def($1, *$2, $4, $6); } 
...
declaration 					:	TYPE T_SEMICOLON	{ $$ = new Declaration(*$1); }
								| 	TYPE initialisation_declarator_list T_SEMICOLON 	{ $$ = new Declaration(*$1, $2); }
...
declarator						: 	T_IDENTIFIER 	{ $$ = new Variable_Declarator(*$1); }
								...
								|	T_IDENTIFIER T_LBRACKET parameter_list T_RBRACKET 	{ $$ = new Function_Prototype_Declaration(*$1, $3); }
...
initialisation_declarator		: 	declarator	{ $$ = $1; }
								...
								| 	declarator T_ASSIGN T_CURLY_LBRACKET initialisation_list T_CURLY_RBRACKET	{ $$ = new Initialisation_Array_Declarator($1, $4); }
								| 	declarator T_ASSIGN T_CURLY_LBRACKET T_CURLY_RBRACKET	{ $$ = new Initialisation_Array_Declarator($1, new std::vector<Expression*>); }
...
enumerator						:	TYPE T_IDENTIFIER T_CURLY_LBRACKET enumerator_list T_CURLY_RBRACKET T_SEMICOLON		{ $$ = new Declaration(*$1, $4); } 

enumerator_list					: 	enumerator_initialisation	{ $$ = new std::vector<Declarator*>(1, $1); }
								| 	enumerator_list T_COMMA enumerator_initialisation	{ $1->push_back($3); $$ = $1; }

cast_expression					: 	unary_expression 
								| 	T_LBRACKET TYPE T_RBRACKET cast_expression	{ $$ = new Cast_Expression(*$2, $4); }

postfix_expression				:	primary_expression	
								|	postfix_expression T_INCREMENT	{ $$ = new Post_Increment_Expression($1, new Direct_Assignment($1, new Add_Expression($1, new Integer(1)))); }
								|	postfix_expression T_DECREMENT	{ $$ = new Post_Decrement_Expression($1, new Direct_Assignment($1, new Decrement_Expression($1, new Integer(1)))); }
...
multiply_expression				:	cast_expression		{ $$ = $1; }
								| 	multiply_expression T_MULTIPLY cast_expression 		{ $$ = new Multiply_Expression($1, $3); }
...
assignment_expression			: 	logical_OR_expression 
								...
								|	unary_expression T_ADD_ASSIGN assignment_expression 	{ $$ = new Direct_Assignment($1, new Add_Expression($1, $3)); }	
								|	unary_expression T_SUB_ASSIGN assignment_expression 	{ $$ = new Direct_Assignment($1, new Sub_Expression($1, $3)); }	

argument_list					: 	assignment_expression	{ $$ = new std::vector<Expression*>(1, $1); }
								| 	argument_list T_COMMA assignment_expression 	{ $1->push_back($3); $$ = $1; }
...
condition_statement 			:	T_IF T_LBRACKET expression T_RBRACKET statement		{ $$ = new Condition_If_Statement($3,$5); }	
								...
								|	T_SWITCH T_LBRACKET expression T_RBRACKET statement		{ $$ = new Switch_Statement($3,$5); }
...
labeled_statement				:	T_CASE expression T_COLON statement 	{ $$ = new Case_Statement($2, $4); }
								| 	T_DEFAULT T_COLON statement 	{ $$ = new Default_Statement($3); } 
	
TYPE							:	T_INT 	{ $$ = new type_def(INT); }
								|	T_VOID	{ $$ = new type_def(VOID); } 	
								|	T_CHAR	{ $$ = new type_def(CHAR); } 		
								|	T_DOUBLE	{ $$ = new type_def(DOUBLE); } 		
								|	T_FLOAT		{ $$ = new type_def(FLOAT); } 	
								|	T_UNSIGNED 	{ $$ = new type_def(UNSIGNED_INT); } 	
								|	T_UNSIGNED T_INT 	{ $$ = new type_def(UNSIGNED_INT); } 
								|	T_ENUM 	{ $$ = new type_def(INT); } 
								| 	TYPE T_MULTIPLY   { $$ = new type_def("POINTER", *$1); }

bitwise_shift_expression		:	add_expression
								|	bitwise_shift_expression T_LEFT_SHIFT add_expression  	{ $$ = new Left_Bitwise_Shift_Expression($1, $3); }
								|	bitwise_shift_expression T_RIGHT_SHIFT add_expression	{ $$ = new Right_Bitwise_Shift_Expression($1, $3); }

compare_expression				: 	bitwise_shift_expression

								|	compare_expression T_LESS bitwise_shift_expression		{ $$ = new Less_Than_Expression($1, $3); }
								|	compare_expression T_LESS_EQUAL bitwise_shift_expression	{ $$ = new Less_Than_Equal_Expression($1, $3); }
								|	compare_expression T_GREATER bitwise_shift_expression	{ $$ = new More_Than_Expression($1, $3); }
								|	compare_expression T_GREATER_EQUAL bitwise_shift_expression		{ $$ = new More_Than_Equal_Expression($1, $3); }
...
enumerator_initialisation		: 	enumerator_declarator T_ASSIGN assignment_expression	{ $$ = new Initialisation_Enum_Declarator($1, $3); }
								|	enumerator_declarator	{ $$ = new Initialisation_Enum_Declarator($1); }

enumerator_declarator			: 	T_IDENTIFIER	{ $$ = new Variable_Declarator(*$1); }

expression 						:	assignment_expression
									
primary_expression				: 	T_INT_CONSTANT	{ $$ = new Integer($1); }
								|	T_FLOAT_CONSTANT	{ $$ = new Float($1); }
								|	T_DOUBLE_CONSTANT	{ $$ = new Double($1); }
								|	T_UNSIGNED_CONSTANT		{ $$ = new Unsigned_Integer($1); }
								| 	T_IDENTIFIER	{ $$ = new Identifier(*$1);	}									
								| 	T_LBRACKET expression T_RBRACKET	{ $$ = $2; }									
								|	T_SIZEOF T_LBRACKET TYPE T_RBRACKET 	{ $$ = new Sizeof_Type_Expression(*$3); }									
								|	T_SIZEOF T_LBRACKET T_IDENTIFIER T_RBRACKET 	{ $$ = new Sizeof_Variable_Expression(*$3); }

unary_expression				: 	postfix_expression
								|	T_BITWISE_AND cast_expression 	{ $$ = new Reference_Expression($2); }								
								|	T_MULTIPLY cast_expression 	{ $$ = new Dereference_Expression($2); }
								|	T_MINUS cast_expression 	{ $$ = new Pre_Negative_Expression($2); }
								|	T_INCREMENT unary_expression 	{ $$ = new Direct_Assignment($2, new Add_Expression($2, new Integer(1))); }
								|	T_DECREMENT unary_expression 	{ $$ = new Direct_Assignment($2, new Sub_Expression($2, new Integer(1))); }
